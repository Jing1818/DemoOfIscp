...
class Captcha{
    ...
    {
        $this->length = $config['length'] ?? config('captcha.length', $this->length);
    ...
    public function create(array $generator = null){
       if (!$generator){
...
//            imagettftext($this->im, $this->fontSize, $angle, $x, $y, $this->color, '', $char);
        }
        ...
        return Response::success($content)->header('Content-Length',strlen($content))->header('Content-Type','image/png');
    }
